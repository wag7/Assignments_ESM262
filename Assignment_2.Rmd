...
```{r 7}
center_county -> cleangaz %>%
  group_by(COUNTY_NAME) %>%
  summarise(latitude = mean(PRIM_LONG_DEC, na.rm = TRUE),
            longitude = mean(PRIM_LAT_DEC, na.rm = TRUE),
            longmax = max(PRIM_LONG_DEC, na.rm = TRUE),
            longmin = min(PRIM_LAT_DEC, na.rm = TRUE),
            latmax = max(PRIM_LONG_DEC, na.rm = TRUE),
...
county_center <- cleangaz
 county_center <- group_by(COUNTY_NAME)
  summarise(
    latMin = min(PRIM_LAT_DEC, na.rm= TRUE),
